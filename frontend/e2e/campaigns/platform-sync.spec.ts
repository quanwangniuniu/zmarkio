...
const RECONNECT_PATH = '/api/facebook_integration/connect/';
...
test('auth banner starts the provider OAuth flow', async ({ page }) => {
  ...
  expect(oauthRequest.method()).toBe('GET');
  expect(new URL(oauthRequest.url()).searchParams.get('project_id')).toBe(String(PROJECT.id));
...
test('failed OAuth initiation keeps the warning and allows another attempt', async ({ page }) => {
  ...
  await page.getByRole('button', { name: 'Reconnect', exact: true }).click();
  await expect(page.getByText('Unable to start reconnection. Please try again.')).toBeVisible();
  await expect(page.getByLabel('Campaign sync warnings')).toBeVisible();
...
test('transient sync failure does not ask the user to reconnect', async ({ page }) => {
